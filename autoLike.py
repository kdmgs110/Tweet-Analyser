...
def likeTweets(tweets, keyword_id):
    ...
    for tweet in tweets:
        ...
        try:
            ...
            print(created_at + " [INFO]likeレコードを追加しました。user_id:{}".format(user_id))
            like_count += 1
            print(created_at + " [INFO]いいね数: {}".format(like_count))
        except Exception as e:
            print(created_at + " [ERROR]いいねに失敗しました: {}".format(e))
            if e.response and e.response.status == 88:
                print(created_at + " [INFO] rate limitの上限値を超えたので、15分待機後に実行します。")
                time.sleep(60 * 15)
            if e.response and e.response.status == 139:
                print(created_at + " [ERROR] すでにいいねをしているツイートです")
    return like_count
